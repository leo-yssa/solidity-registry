...
import type { ERC20UUPS } from '../../lib/interfaces';
...
describe('ERC20UUPS', function () {
    it('Should deploy, initialize, and upgrade correctly', async function () {
        ...
        const proxy = (await upgrades.deployProxy(UUPSFactory, ['MyTokenUUPS', 'MTU', ethers.parseEther('1000')], {
            kind: 'uups',
        })) as ERC20UUPS;
        ...
        const upgradedProxy = await upgrades.upgradeProxy(await proxy.getAddress(), UUPSFactory);
